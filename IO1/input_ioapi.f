...
c     call READ_IOAPI('INITF',sname(l,1),                   ! inital condition in ppb      
c    1  1998,5,15,0,sg1(1,1,1,l),iflag)
       if(iflag.eq.1) then
...
c                                              read emissions
     ...
     1     2001,1,1,ihr, em(1,1,1,L),iflag2)   ! our emission already in molecular/cm2
     
        if(iflag2.eq.1) call aq_zero_r4(ix*iy*iz,em(1,1,1,L))
	iflag=iflag*iflag2
       endif	
	 ...
	 do j=1,iy
	  ...
	  enddo 
	  if(emname(L).eq.'SSF'.or.emname(L).eq.'SSC') 
...
c      do i=1,ix
c      do k=7,iz
c      sx(i,k,1,7)=50.*sg1(i,1,k,iair)*1.e-09
c      sx(i,k,2,7)=50.*sg1(i,iy,k,iair)*1.e-09
c      enddo
c      enddo
cc
c      do j=1,iy
c      do k=7,iz
c      sy(j,k,1,7)=50.*sg1(1,j,k,iair)*1.e-09
c      sy(j,k,2,7)=50.*sg1(ix,j,k,iair)*1.e-09
c      enddo
c      enddo
cc
      do l=1,numsp
...
c       sx(j,iz-2,1,io3)=amax1(30.,pv(1,j,iz-2)*1e6*pvo3)*
c     1   sg1(1,j,iz-2,iair)/1e9
c       sx(j,iz-3,1,io3)=amax1(30.,pv(1,j,iz-3)*1e6*pvo3)*
c     1   sg1(1,j,iz-3,iair)/1e9
       
       sx(j,iz,2,io3)=sz(ix,j,io3)                       !east
       sx(j,iz-1,2,io3)=sz(ix,j,io3+numsp)                 
c       sx(j,iz-2,2,io3)=amax1(30.,pv(ix,j,iz-2)*1e6*pvo3)*
c     1   sg1(ix,j,iz-2,iair)/1e9
c       sx(j,iz-3,2,io3)=amax1(30.,pv(ix,j,iz-3)*1e6*pvo3)*
c     1   sg1(ix,j,iz-3,iair)/1e9
      enddo
      
      do i=1,ix 
       sy(i,iz,1,io3)=sz(i,1,io3)                        !south
       sy(i,iz-1,1,io3)=sz(i,1,io3+numsp)
c       sy(i,iz-2,1,io3)=amax1(30.,pv(i,1,iz-2)*1e6*pvo3)*
c     1   sg1(i,1,iz-2,iair)/1e9
c       sy(i,iz-3,1,io3)=amax1(30.,pv(i,1,iz-3)*1e6*pvo3)*
c     1   sg1(i,1,iz-3,iair)/1e9
       
       sy(i,iz,2,io3)=sz(i,iy,io3)                       !north
       sy(i,iz-1,2,io3)=sz(i,iy,io3+numsp)                 
c       sy(i,iz-2,2,io3)=amax1(30.,pv(i,iy,iz-2)*1e6*pvo3)*
c     1   sg1(i,iy,iz-2,iair)/1e9
c       sy(i,iz-3,2,io3)=amax1(30.,pv(i,iy,iz-3)*1e6*pvo3)*
c     1   sg1(i,iy,iz-3,iair)/1e9
      enddo
...
c      do i=1,ix
c      do j=1,iy
c      sz(i,j,7)=60.*sg1(i,j,iz,iair)*1.e-09
c      enddo
c      enddo
