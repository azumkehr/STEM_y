...
c     c   Just do the forward with reading in scaling facotrs
c
c
...
c     do isen = 1, ixm
c     do jsen = 1,iym
c     read(27,*) emi_fac(isen,jsen,1,1)
c     print*, isen,jsen,emi_fac(isen,jsen,1,1)
c     enddo
c     enddo
c     close(27)
c
...
c     +++++  Comment this section out when doing emission inversion ++++++
c     c   Compute the forward model and cost function for getting emi_grd
c
c     do i=1,lbfgs_n/2
c     emi_fac(i_map(i),j_map(i),1,1)=xoptim(i)
c     emi_fac(i_map(i),j_map(i),2,1)=xoptim(i+lbfgs_n/2)
...
!==============================================================
     ...
     &           +(emi_fac(i_map(i),j_map(i),2,1)-1.)
     &           /xuncert(lbfgs_n/2+i)**2
         enddo
...
c--------------------------------------------------------------------------
     ...
     &        MAXVAL(abs(gradient((i_ctrl-1)
     &        *Num_grid+1:i_ctrl*Num_grid)))
         write(urpt,*) 'RMS of GRAD',
     &        sqrt(SUM(gradient((i_ctrl-1)
     &        *Num_grid+1:i_ctrl*Num_grid)**2)/
     &        dble(Num_grid))
      enddo
      write(urpt,*)
