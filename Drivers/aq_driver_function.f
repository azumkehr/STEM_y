...
c	print*,'DEBUG postmodify KV', kv(40,30,2)
         ...
         if (it.eq.1) then
	 sg1(:,:,:,is_em)=sg1(:,:,:,is_em)/sg1(:,:,:,iair)
         call prtout(idate,ut,ix,iy,iz,sg1
     &                ,sl1,iout) !output initial condition
	  sg1(:,:,:,is_em)=sg1(:,:,:,is_em)*sg1(:,:,:,iair)
        end if  ! (it.eq.1)
     ...
     &          /( (unc_obs(i_mea)*ave_obs(i_mea)/1.e9)**2 )           
      ...
      !   files are written in "ppbv" following convention. 
        enddo  
            ...
            do i=1,ix
            do j=1,iy
                write(unit_pred1,*) i,j,emi_fac(i,j,1,:)
            enddo
            enddo
...
125   FORMAT (3e13.5)
     ...
     &   ,minval(emi_fac(:,:,1,1))
        ...
        write(*,*) 'Before2 write costfct:', costfct
        open(unit=unit_cost, file='costfct' )
        write( unit_cost,*) costfct
        close( unit_cost )
        write(*,*) 'After writing the file' 
