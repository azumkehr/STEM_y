ccc g77 -fno-silent disads99_ros2.f adjros2.f saprcnov_stem_model.f 
...
c discrete adjoint backward integration	
	do while (istep .gt. 1)	            
...
c restore forward variables
          do i=1,nvar
            var(i) = fwdtraji(istep-1,i)	 
          enddo	  
c backward integration 
          call DADINTEGRATE(advar,h,time)
          istep = istep - 1
	write(12,999)(time-tstart)/3600.0d0,(advar(i),i=1,nvar)  
        end do  ! istep loop  
...
cccc begin finite differences check 
      pert = 1.d-6
      open(15, file = 'dadjsensit_ros2.dat') 
...
999   FORMAT(E24.16,100(1X,E24.16))
     ...
     &              ' at time = ',time
      ...
      SUBROUTINE DADINTEGRATE(ady,h,t)
      	
      INCLUDE 'saprcnov_stem_s.h' 
      REAL*8 ady(nvar)
      REAL*8 h,t
      
      call dadj_ros2(var,ady,h,t) 
