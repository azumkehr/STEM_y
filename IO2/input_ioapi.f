...
c        enddo
c        close(27)
c	print*, 'DEBUG vcosco2',maxval(vcosco2),minval(q(:,:,L))
c	q(:,:,L) = vcosco2 * q(:,:,L)
c        print*, 'DEBUG vcosco2',maxval(vcosco2),minval(q(:,:,L))
c
c	endif
...
c      write(0,*) 'read variable ',varname,date3d,time3d,' from ',lname
	...
	write(6,*)'time =', date3d, time3d
      ...
      if (.not. DESC3(trim(lname)) ) then
       ...
       iflag = 1
        CALL M3WARN( 'READ_IOAPI', date3d, time3d,
     &       'DESC3 failed' )
      endif
...
c------------------------------------------------------------------------------
      ...
      if (.not. DESC3(trim(lname)) ) then
         print*,'No information for',lname
         iflag = 1
         CALL M3WARN( 'READ_IOAPI_FIND_PREVIOUS', date3d, time3d,
     &       'DESC3 failed' )
      endif
